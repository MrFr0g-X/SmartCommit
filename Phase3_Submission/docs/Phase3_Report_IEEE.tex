...
\textbf{Layer 6 - Sensitive Data Detection:} Scans for 6 patterns using regex (password/API key/secret/token assignments, 16-digit credit cards, email addresses). Patterns detect common sensitive data in code changes to prevent accidental exposure in version control.
...
\subsection{Alternative Implementation: CrewAI Framework}
...
\subsubsection{CrewAI Architecture}

The CrewAI implementation (separate project: \texttt{ethical\_ai\_commit\_message\_generator\_with\_governance\_v1}) provides identical functionality using declarative configuration and framework-managed orchestration:

\textbf{Agent Configuration (YAML):} Three agents defined in \texttt{config/agents.yaml} with roles, goals, and backstories. Each agent uses Gemini 2.0 Flash Experimental (temperature=0.1) via CrewAI's LLM abstraction layer.

\textbf{Task Configuration (YAML):} Four tasks in \texttt{config/tasks.yaml}: (1) generate\_initial\_commit\_message, (2) validate\_quality\_and\_safety, (3) refine\_message\_with\_governance, (4) final\_governance\_output. Tasks include descriptions, expected outputs (JSON schemas), and context dependencies.

\textbf{Crew Orchestration:} Sequential workflow managed by CrewAI with built-in progress tracking, error handling, and execution tracing.

\subsubsection{CrewAI Test Results}

Live testing with sample diff (\texttt{@@ api/utils.py @@} function rename) produced:

\textbf{Generated Message:} "Fix: Rename calculate function to calculate\_total and modify to add two numbers in api/utils.py"

\textbf{Agent Trail:}
\begin{itemize}
    \item GeneratorAgent: 10.5ms execution, reasoning provided
    \item ValidatorAgent: 5.2ms execution, ACCEPT decision (quality\_score=0.95, BLEU=0.92, ROUGE-L=0.96, semantic=0.98)
    \item RefinerAgent: 0ms (no refinement needed - validator accepted)
\end{itemize}

\textbf{Governance Compliance:}
\begin{itemize}
    \item Safety validated: true (hallucination\_severity=NONE, confidence=HIGH)
    \item Transparency enabled: true (all parameters logged)
    \item Explainability provided: true (all agents provided reasoning)
    \item Accountability traced: true (complete audit trail with timestamps)
    \item Compliance score: 1.0 (100\%)
\end{itemize}

\textbf{Total Execution Time:} 15.7ms (agent processing only; excludes Gemini API latency)

\textbf{Iterations:} 1 (no refinement loop triggered)

\subsubsection{Framework Comparison}

\begin{table}[h]
\centering
\caption{Custom vs CrewAI Implementation Comparison}
\label{tab:framework_comparison}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Feature} & \textbf{Custom} & \textbf{CrewAI} \\
\midrule
Code (LOC) & 590 & $\sim$200 \\
...
\textbf{Key Insight:} Both implementations achieve identical governance outcomes (100\% compliance), validating that ethical AI principles can be implemented across frameworks. CrewAI offers faster development (200 vs 590 LOC) and industry-standard patterns, while custom implementation provides maximum integration with existing SafetyGuardrails infrastructure.
